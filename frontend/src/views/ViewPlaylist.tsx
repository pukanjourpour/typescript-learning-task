...
export default class ViewPlaylist extends React.Component<Props, State> {
	constructor(props: Props) {
		...
		this.state = {
			...
			selectedSongId: null,
			isOwner: this.props.userUuid === this.props.selectedPlaylist.playlist_user_uuid,
	...
	handleCreate = async (event: React.FormEvent<HTMLFormElement>, newSongTitle: string, newSongArtist: string, newSongAlbum: string, newFileB64: string) => {
		...
		let playlistId = this.props.selectedPlaylist.playlist_id ? this.props.selectedPlaylist.playlist_id : -1;
		let result = await ControllerSongs.addSong(this.props.sessionHash, this.props.userUuid, playlistId, newSongTitle, newSongArtist, newSongAlbum, newFileB64);
	...
	handleDelete = async (song: Song) => {
		if (song.song_id && song.song_user_uuid === this.props.userUuid) {
			let result = await ControllerSongs.deleteSong(this.props.sessionHash, this.props.userUuid, song.song_id);
	...
	render = () => {
		return (
			<Grid container justifyContent={"center"} pt={3} columns={2} columnSpacing={3}>
				<Grid container item rowSpacing={3} xs={8}>
					<Grid container item xs={8} direction={"column"}>
						<Grid item xs={5}>
							<List>
								<ListSubheader><Typography
									variant={"h4"}>{this.props.selectedPlaylist.playlist_title} by {this.props.selectedPlaylistAuthor}</Typography></ListSubheader>
								{this.state.songs?.map((song) =>
									<ListItem key={song.song_id} secondaryAction={
										this.state.isOwner ?
											<IconButton onClick={() => this.handleDelete(song)} edge="end"
																	aria-label="delete"><Delete /></IconButton>
											: null
									}>
										<ListItemButton onClick={() => {
											...
											<ListItemText primary={song.song_title} secondary={
												<React.Fragment>
													<Typography
														sx={{ display: "inline" }}
														component="span"
					...
					<Grid item xs={4}>
						<Grid container direction={"column"}>
							...
							<hr/>
							{this.state.isOwner ?
								<FormAddSong songAddedMessage={this.state.songAddedMessage} handleCreate={this.handleCreate}></FormAddSong> : null}
						</Grid>
