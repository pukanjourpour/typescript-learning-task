...
export default class ViewMyPlaylists extends React.Component<Props, State> {
	...
	// All client-server communication errors are intercepted by a try-catch block in axiosPost function,
	// which will return null as a response if any error occurs.
	// In the component every response is then checked if it is null or not and if it is successful, therefore,
	// no try-catch blocks are needed in the components.

	handleCreate = async (event: React.FormEvent<HTMLFormElement>) => {
		event.preventDefault();
		// TODO: Playlist info validation
		let result = await ControllerPlaylists.createPlaylist(
			this.props.sessionHash,
			this.props.userUuid,
			this.state.newPlaylistTitle,
	...
	handleDelete = async (playlistId: number | null) => {
		// TODO: Playlist info validation
		if (playlistId) {
			let result = await ControllerPlaylists.deletePlaylist(
				this.props.sessionHash,
				this.props.userUuid,
				playlistId,
			);
	...
	componentDidMount = async () => {
		if (this.props.authenticated) {
			let result = await ControllerPlaylists.getUserPlaylists(
				this.props.sessionHash,
				this.props.userUuid,
			);
			if (result) {
	...
	render = () => {
		...
		if (this.props.authenticated) {
									...
									<List>
										<ListSubheader>
											<Typography variant={"h4"}>
												{i18next.t("my-playlists").toString()}
											</Typography>
										</ListSubheader>
										{this.state.playlists.map((playlist) => (
											<ListItem
												...
												secondaryAction={
													<IconButton
														onClick={() =>
															this.handleDelete(playlist.playlist_id)
														}
														edge="end"
														aria-label="delete"
													>
											...
											>
												...
												>
													...
													<ListItemText
														primary={playlist.playlist_title}
									...
									>
										<Grid item>
											<Typography variant={"h5"}>
												{i18next.t("create-new-playlist").toString()}
											</Typography>
										</Grid>
										<Grid item xs={8}>
											<Input
												...
												onChange={(val) => {
													this.setState({
														newPlaylistTitle: val.currentTarget.value,
													});
												}}
												placeholder={i18next.t("create-new-playlist-title").toString()}
										...
										<Grid item xs={8}>
											<Input
												...
												onChange={(val) => {
													this.setState({
														newPlaylistDescription: val.currentTarget.value,
													});
												}}
												placeholder={i18next.t("create-new-playlist-description").toString()}
										...
										<Grid item>
											<Button type="submit" variant="contained" color="success">
												<Typography variant={"button"}>
													{i18next.t("create-new-playlist-btn").toString()}
												</Typography>
											</Button>
										</Grid>
									</Grid>
			...
			content = (
				<Typography variant={"h3"} mt={"1rem"} align={"center"}>
					{i18next.t("login-required-playlists").toString()}
				</Typography>
			);
		}
		return content;
