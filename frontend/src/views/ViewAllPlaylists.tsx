...
export default class ViewAllPlaylists extends React.Component<Props, State> {
  ...
  async componentDidMount() {
    if (this.props.authenticated) {
      let result = await ControllerPlaylists.getAllPlaylists(
  ...
  initAuthors = async (playlists: Playlist[]): Promise<string[]> => {
    ...
    for (let playlist of playlists) {
      let result = await ControllerUsers.getUsernameByUuid(
        this.props.userUuid,
        this.props.sessionHash,
        playlist.playlist_user_uuid
      );
      if (result) {
                ...
                <List>
                  <ListSubheader>
                    <Typography variant={"h4"}>
                      {i18next.t("browse-playlists").toString()}
                    </Typography>
                  </ListSubheader>
                    ...
                    <ListItemButton
                      ...
                      onClick={() => {
                        this.setState({
                          ...
                          selectedPlaylistAuthor: this.state.authors[index],
                        });
                      }}
                    >
                      <ListItem>
